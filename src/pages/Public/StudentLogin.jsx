...
function StudentLogin() {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-3xl rounded-[32px] bg-white p-10 shadow-soft dark:bg-slate-950">
      <div className="mb-8">
        <p className="text-sm font-semibold uppercase tracking-[0.3em] text-secondary">Student Login</p>
        <h1 className="mt-4 text-3xl font-semibold text-slate-950 dark:text-white">Access your student dashboard</h1>
        <p className="mt-3 text-slate-600 dark:text-slate-300">Log in with your student number to review announcements, assignments, and your timetable.</p>
      </div>

      <form className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Student Number</label>
          <input type="text" placeholder="e.g. S1234567" className="mt-3 w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100" />
        </div>
        <div>
        ...
        <p className="text-center text-sm text-slate-600 dark:text-slate-400">
          No account? <Link to="/student-register" className="font-semibold text-primary">Register here</Link>
        </p>
      </form>
    </motion.div>
  );
